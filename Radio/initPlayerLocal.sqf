//This file gets executed on the moment a player joins the server.
IncomingSound = {
...
[] spawn {
	while{true} do {
		...
		while{inputAction "PushToTalk" > 0} do {
			if(first)then{
				playSound "soundRadioSendingClick";
				playSound "soundRadioSendingStart";
				first = false;
				_id = -(clientOwner);
			 	remoteExec ["IncomingSound", _id];
				sleep 1;
			};
		};
	};
